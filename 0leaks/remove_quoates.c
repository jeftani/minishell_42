...
/*   Updated: 2023/07/25 03:36:20 by gloukas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_lexer	*last_expand(t_lexer **lexer, t_env *environment)
{
	t_lexer *head;
	int i;
	...
	while (head)
	{
		if (head && head->command)
			head->command = expand(head->command, environment, head);
		while (head && head->details && head->details->string
			&& head->details->string[++i])
			head->details->string[i] = expand(head->details->string[i],
												environment,
												head);
		i = -1;
		while (head && head->details && head->details->string
			&& head->details->string[++i] && head->details->string[i][0] == '$')
			head->details->string[i] = expand(head->details->string[i],
												environment,
												head);
		head = head->next;
