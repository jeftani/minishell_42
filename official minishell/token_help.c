...
/*   Updated: 2023/07/28 16:13:13 by ajeftani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_cmd	*creation_a(t_cmd *details, t_count count)
{
	...
	if (count.string)
	{
		details->string = (char **)ft_malloc((count.string + 1) * sizeof(char *));
		details->string[count.string] = NULL;
	}
	if (count.r_in)
	{
		details->r_in_sig = (char **)ft_malloc((count.r_in + 1) * sizeof(char *));
		details->r_in_file = (char **)ft_malloc((count.r_in + 1) * sizeof(char *));
